...
- 이러한 react native의 특성을 이용하여 HomeScreen에선 처음 screen을 방문했을 때 유저 위치 기반으로 다이어리를 필터링한 후, 스크린을 나가도 unmount 처리하지 않고 해당 스크린을 유지합니다. 유저가 현위치를 업데이트하고 다이어리를 다시 찾고 싶은 경우에만 버튼을 눌러 업데이트 할 수 있도록 구현했습니다. 다른 screen에선 해당 스크린에서 나가면 navigator Screen에 옵션에 `unmountOnBlur` 을 적용하여 `componentWillUnmount`를 적용해주었고, navigation 외에서 조작을 원할 경우 `useIsFocused` hook을 사용하여 조절해주었습니다.
